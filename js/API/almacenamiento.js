...
function leerrHistorial(){
accesoBD().transaction(function(tx){
	tx.executeSql('SELECT * FROM historial',[],function(txl,resultado){
		...
		if(largo!=0){
            ...
            '<div data-role="collapsible" data-collapsed="true">'+
				...
				'Dias: '+resultado.rows.item(i).dias+'<br />'+
...
function leerReservas(){
	accesoBD().transaction(function(tx){
		tx.executeSql('SELECT * FROM reservaciones',[],function(txl,resultado){
		var largo = resultado.rows.length;
		if(largo!=0){
		    for(i=0;i<largo;i++){
